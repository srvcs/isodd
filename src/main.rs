use srvcs_isodd::{api::Deps, config::Config, health, router, telemetry};
...
async fn main() {
    ...
    tracing::info!(addr = %cfg.bind_addr, env = %cfg.environment, "srvcs-isodd listening");
    let deps = Deps {
        iseven_url: cfg.iseven_url.clone(),
    };
